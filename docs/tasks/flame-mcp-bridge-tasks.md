...
- [x] Set up a basic `server.py` using `fastmcp`.
- [x] Create a README in `flame-mcp/` explaining how to start the server.

## Phase 2: Core Communication Layer
- [x] Implement the `Relay` client: a module to handle network communication with the existing `flame-listener.py` (port 5555).
- [x] Standardize the error handling for relay failures (e.g., Flame is not running, Listener is down).
- [x] Implement a "Ping" tool to verify the end-to-end connection: `AI -> MCP -> Listener -> Flame`.
...
- [x] Write a "Getting Started" guide for users to connect their LLM to the Flame MCP.
